...
const getSignerStatus = (record: CnsdTransmitterMeterRecordSummary, userName: string | undefined, userRole: string | undefined): SignerBadgeStatus => {
...
export const CnsdTransmitterMeterListPage: React.FC = () => {
  ...
  useEffect(() => {
    cnsdTransmitterMeterService
      .getYears(config.formType)
  ...
  const fetchRecords = useCallback(async () => {
    ...
    try {
      ...
      const response = await cnsdTransmitterMeterService.listRecords(params, config.formType);
  ...
  }, [config.formType, debouncedSearch, dateFilter, yearFilter, shiftFilter, statusFilter]);
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      navigate(`${config.basePath}/${record.id}`);
    } catch (err) {
      if (axios.isAxiosError(err) && err.response) {
        ...
        if (err.response.status === 409 && data.errors?.existing_record) {
          navigate(`${config.basePath}/${data.errors.existing_record.id}`);
          return;
        }
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await cnsdTransmitterMeterService.deleteRecord(id);
      void fetchRecords();
    } catch {
        ...
        <div className="flex gap-2">
          <div className="relative flex-1">
            ...
            <input
              ...
              aria-label="Cari Meter Reading Transmitter"
            />
          </div>
          {hasActiveFilters && (
            <button
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 space-y-3 px-4 text-center">
            ...
            <p className="text-base font-semibold text-slate-700">Belum ada Meter Reading </p>
            <p className="text-sm text-slate-400 max-w-md">
              Klik <strong>Buat Meter Reading</strong> untuk membuat form VHF ER Gedung Radar Baru untuk shift hari ini.
            </p>
            {canCreate && (
              ...
              <tbody>
                {records.map((r) => {
                  ...
                  return (
                    <tr
                      key={r.id}
                      onClick={() => navigate(`${config.basePath}/${r.id}`)}
                      tabIndex={0}
                      role="button"
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-center">
                        <div className="flex items-center justify-center gap-2" onClick={(e) => e.stopPropagation()}>
                          <button onClick={() => navigate(`${config.basePath}/${r.id}`)} className="p-1.5 text-slate-400 hover:text-amber-500 hover:bg-amber-50 rounded-lg transition-colors" title="Detail / Edit">
                            <Edit2 size={16} />
                          </button>
                          <button onClick={() => navigate(`${config.basePath}/${r.id}/print`)} className="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors" title="Print PDF">
                            <Printer size={16} />
                          </button>
                          {canDelete && (
                            <button onClick={() => handleDelete(r.id)} className="p-1.5 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors" title="Hapus">
                              <Trash2 size={16} />
                            </button>
                          )}
                        </div>
