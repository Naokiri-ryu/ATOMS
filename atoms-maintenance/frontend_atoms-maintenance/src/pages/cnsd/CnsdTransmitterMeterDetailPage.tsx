...
import { AlertCircle, ArrowLeft, Calendar, Clock, Lock as LockIcon, MapPin, Printer, Radio as TransmitterIcon, Save, Users } from "lucide-react";
...
export const CnsdTransmitterMeterDetailPage: React.FC = () => {
  ...
  const fetchRecord = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const data = await cnsdTransmitterMeterService.getRecord(recordId, config.formType);
      setRecord(data);
      setEditedItems({});
  ...
  }, [config.formType, recordId]);

  useEffect(() => {
    void fetchRecord(); /* eslint-disable-next-line react-hooks/exhaustive-deps */
  }, [recordId]);
  ...
  if (!record) {
    return (
      <div className="max-w-3xl mx-auto py-20 text-center space-y-4">
        ...
        <Button variant="outline" onClick={() => navigate(config.basePath)} className="gap-2">
  ...
  return (
    <div className="max-w-full space-y-5 animate-fade-in pb-12">
      <div className="flex items-center gap-2 text-xs text-slate-500">
        ...
        <span>/</span>
        <button type="button" onClick={() => navigate(config.basePath)} className="hover:text-slate-700 transition-colors">
          {config.title}
        </button>
        <span>/</span>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs text-slate-600">
            ...
            <Button variant="ghost" size="sm" onClick={() => navigate(`${config.basePath}/${record.id}/print`)} className="gap-1.5 text-indigo-600 hover:bg-indigo-50">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
        ...
        <p className="text-[10px] text-slate-400 italic mt-3">Identifikasi peralatan (Merk) tercatat per channel di tabel frekuensi.</p>
      </div>
      ...
      {activeSectionMeta && <TransmitterSectionPanel sectionMeta={activeSectionMeta} items={itemsBySection[activeSectionMeta.code] ?? []} isReadOnly={!!isReadOnly} getValue={getValue} onChange={updateField} />}
      ...
      <CnsdTransmitterMeterSignaturePanel record={record} onRecordUpdate={(r) => setRecord(r)} formType={config.formType} />
    </div>
  );
};
    ...
    if (!isBackup && freq && freq !== lastFreq) {
      rows.push(
        <tr key={`freq-${item.id}`} className="bg-emerald-50/40">
          ...
          <td colSpan={4} className="border-b border-slate-100">
            &nbsp;
          </td>
        </tr>,
      );
...
const TransmitterItemRow: React.FC<TransmitterItemRowProps> = ({ item, isBackup, isReadOnly, getValue, onChange }) => {
  ...
  return (
    <tr className="hover:bg-slate-50 transition-colors border-b border-slate-100">
      ...
      <td className="px-3 py-2 align-middle text-slate-800 font-medium">{isBackup ? item.frequency_label : (item.tx_label ?? "")}</td>
      <td className="px-2 py-2 align-middle text-center text-slate-600 text-[11px]">{isBackup ? (item.merk ?? "—") : ""}</td>
      {item.is_blocked ? (
