...
import type { CnsdTransmitterMeterRecordDetail, CnsdTransmitterMeterTechnician } from "@/types/cnsdTransmitter";
import { TRANSMITTER_FORM_TYPE } from "@/services/cnsdTransmitterMeterService";
...
export const CnsdTransmitterMeterSignaturePanel: React.FC<Props> = ({ record, onRecordUpdate, formType = TRANSMITTER_FORM_TYPE }) => {
  const { user } = useAuth();
  ...
  type Column = { kind: "manager" | "supervisor"; label: string; info: SignerInfo | null } | { kind: "technician"; label: string; row: CnsdTransmitterMeterTechnician };
  ...
  const handleSign = async (signature: string) => {
    ...
    try {
      const result = await cnsdTransmitterMeterService.signRecord(record.id, pending.role, signature, pending.technicianRowId, formType);
  ...
  const renderColumn = (col: Column, idx: number): React.ReactElement => {
    switch (col.kind) {
      ...
      case "supervisor": {
        ...
        const isAuthorized = col.kind === "manager" ? !!expectedName && namesMatch(expectedName, userName) && requiredRoleOk : !!expectedName && requiredRoleOk; // Supervisor slot: Manager can delegate
          ...
          <div key={`${col.kind}-${idx}`} className="space-y-3">
            ...
            {showLockedNote && (
              <div className="flex items-start gap-2 rounded-md border border-slate-200 bg-slate-50 px-2.5 py-2 text-xs text-slate-600">
                ...
                <span>
                  Tanda tangan hanya dapat dilakukan oleh <span className="font-semibold text-slate-800">{expectedName}</span>
                </span>
              </div>
            )}
          </div>
          ...
          <div key={`tech-${row.id}`} className="space-y-3">
            ...
            {showLockedNote && (
              <div className="flex items-start gap-2 rounded-md border border-slate-200 bg-slate-50 px-2.5 py-2 text-xs text-slate-600">
                <Lock size={12} className="mt-0.5 text-slate-400 shrink-0" />
                <span>
                  Tanda tangan hanya dapat dilakukan oleh <span className="font-semibold text-slate-800">{row.technician_name}</span>
                </span>
              </div>
            )}
          </div>
