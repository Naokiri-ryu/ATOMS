...
import type {
  ...
  TfpGensetRadarSignerInfo,
  TfpGensetRadarTechnicianRow,
...
export const TfpGensetRadarSignaturePanel: React.FC<Props> = ({ record, onUpdated, isDirtyCheck }) => {
  ...
  type Column =
    | { kind: 'manager' | 'supervisor'; label: string; info: TfpGensetRadarSignerInfo | null }
    | { kind: 'technician'; label: string; row: TfpGensetRadarTechnicianRow };
  ...
  const handleSign = async (signature: string) => {
    ...
    try {
      const result = await tfpGensetRadarService.signRecord(
        record.id,
        pending.role,
        signature,
