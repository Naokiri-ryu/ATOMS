...
const getSignerStatus = (
  record: TfpGensetRadarRecordSummary,
...
export const TfpGensetRadarListPage: React.FC = () => {
  ...
  const [records, setRecords] = useState<TfpGensetRadarRecordSummary[]>([]);
  ...
  const fetchRecords = useCallback(async () => {
    ...
    try {
      ...
      const response = await tfpGensetRadarService.listRecords(params);
      ...
      if (axios.isAxiosError(err) && err.response?.status === 401) {
        ...
        setErrorMessage('Gagal memuat daftar Performance Check Genset Radar.');
      }
      setRecords([]);
      setTotalCount(0);
    } finally {
  ...
  const handleCreate = async () => {
    ...
    setErrorMessage(null);
    try {
      const record: TfpGensetRadarRecordDetail = await tfpGensetRadarService.createRecord({
        date: getCurrentShiftDate(),
        shift_type: getCurrentShiftType(),
      });
      navigate(`/tfp/genset-radar/${record.id}`);
    } catch (err) {
      if (axios.isAxiosError(err) && err.response) {
        ...
        setErrorMessage(data.message ?? 'Gagal membuat Performance Check Genset Radar.');
        return;
      }
      setErrorMessage('Gagal membuat Performance Check Genset Radar. Coba lagi.');
    } finally {
      setIsCreating(false);
    }
  };
  ...
  const handleDelete = async (id: number) => {
    if (
      !confirm(
        'Hapus form Performance Check Genset Radar ini? Tindakan ini tidak dapat diurungkan.',
      )
    )
      return;
    try {
      await tfpGensetRadarService.deleteRecord(id);
      void fetchRecords();
    } catch {
  ...
  return (
    <div className="space-y-5 animate-fade-in max-w-7xl mx-auto">
      <div className="flex items-center gap-2 text-xs text-slate-500">
        ...
        <span>/</span>
        <span className="text-slate-700 font-medium">Performance Check Genset Radar</span>
      </div>
      ...
      <PageHeader
        ...
        title="Performance Check Genset Radar"
        ...
        <div className="flex gap-2">
          <div className="relative flex-1">
            ...
            <input
              ...
              aria-label="Cari Performance Check Genset Radar"
            />
          </div>
          {hasActiveFilters && (
            <button
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 space-y-3 px-4 text-center">
            ...
            <p className="text-base font-semibold text-slate-700">
              Belum ada Performance Check Genset Radar
            </p>
              ...
              <tbody>
                {records.map((r) => {
                  ...
                  return (
                    <tr
                      key={r.id}
                      onClick={() => navigate(`/tfp/genset-radar/${r.id}`)}
                      onKeyDown={(e) => {
                        if (e.key === 'Enter' || e.key === ' ') {
                          e.preventDefault();
                          navigate(`/tfp/genset-radar/${r.id}`);
                        }
                      }}
                      tabIndex={0}
                      role="button"
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-center">
                        ...
                        >
                          ...
                          <button
                            onClick={() => navigate(`/tfp/genset-radar/${r.id}/print`)}
