...
// [TAMBAH] Normalisasi string untuk pencarian nama — lowercase + trim,
// dipakai di kedua sisi (query & nama manager) supaya match tidak
// case-sensitive dan tidak terganggu spasi berlebih.
const normalize = (s: string): string => s.trim().toLowerCase();

// [TAMBAH] Cek apakah sebuah logbook punya minimal 1 manager on-duty yang
// namanya cocok (substring match) dengan query pencarian.
const matchesEmployeeSearch = (lb: LogbookCnsdSummary, query: string): boolean => {
...
export const LogbookCnsd: React.FC = () => {
  ...
  // [TAMBAH] Query pencarian nama karyawan (Manager Teknik On Duty)
  ...
  // [TAMBAH] Daftar logbook setelah difilter oleh nama karyawan (client-side).
  ...
  // [UBAH] hasActiveFilters & empty-state sekarang juga memperhitungkan
    ...
    <div className="space-y-5 animate-fade-in max-w-7xl mx-auto">
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-4 space-y-3">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {/* [TAMBAH] Search box nama karyawan (Manager Teknik On Duty) */}
          ...
          {!isLoading && (
            <span className="ml-auto self-center text-xs text-slate-400">
              {/* [UBAH] Tampilkan jumlah hasil setelah filter nama, bukan total mentah,
        ...
        {hasActiveFilters && (yearFilter || signedFilter || employeeSearch) && (
          <div className="flex flex-wrap items-center gap-1.5 pt-1 border-t border-gray-100">
            <span className="text-[11px] text-slate-400 mr-1">Aktif:</span>
            {/* [TAMBAH] chip untuk pencarian nama karyawan */}
            {employeeSearch && (
              <span className="inline-flex items-center gap-1 rounded-full bg-blue-50 border border-blue-200 text-blue-700 text-[11px] font-medium px-2 py-0.5">
                Nama: "{employeeSearch}"
                <button onClick={() => setEmployeeSearch('')} className="ml-0.5 rounded-full hover:bg-blue-100"><X size={10} /></button>
              </span>
            )}
            {yearFilter && (
              <span className="inline-flex items-center gap-1 rounded-full bg-blue-50 border border-blue-200 text-blue-700 text-[11px] font-medium px-2 py-0.5">
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 space-y-3 px-4 text-center">
            ...
            {/* [UBAH] Pesan lebih spesifik kalau yang bikin kosong adalah pencarian nama */}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm min-w-[600px]">
              ...
              <tbody>
                {/* [UBAH] pakai filteredLogbooks, bukan logbooks mentah */}
