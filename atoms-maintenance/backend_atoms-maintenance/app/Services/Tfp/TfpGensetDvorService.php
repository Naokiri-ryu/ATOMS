...
use App\Models\Tfp\TfpGensetDvorFacility;
use App\Models\Tfp\TfpGensetDvorItem;
use App\Models\Tfp\TfpGensetDvorRecord;
use App\Models\Tfp\TfpGensetDvorTechnician;
...
/**
 * TfpGensetDvorService — orchestrates the TFP Performance Check Genset DVOR
 ...
 *   - Items and facilities are seeded from TfpGensetDvorTemplate at create.
 */
class TfpGensetDvorService
{
    ...
    {
        $query = TfpGensetDvorRecord::query()
            ->with([
                'technicians:id,genset_dvor_record_id,technician_id,technician_name,technician_signature,sort_order',
                'manager:id,name',
                'supervisor:id,name',
            ])
    ...
    public function findRecord(int $id): ?TfpGensetDvorRecord
    {
        return TfpGensetDvorRecord::query()
            ->with([
                'technicians',
                'items',
                'facilities',
    ...
    public function findExistingRecord(string $formType, string $date, string $shiftType): ?TfpGensetDvorRecord
    {
        return TfpGensetDvorRecord::query()
    ...
    /**
     * Create a new TFP Genset DVOR record + auto-resolve personnel + seed items + facilities.
     *
     * @throws TfpGensetDvorDuplicateException when a record already exists.
     * @throws RuntimeException when no TFP technicians are on duty.
     */
    public function createRecord(array $data, ?LocalUser $creator = null): TfpGensetDvorRecord
    {
        $formType  = $data['form_type']  ?? 'GENSET-DVOR';
        $date      = $data['date'];
        $shiftType = $data['shift_type'];
        $location  = $data['location']   ?? 'GENSET DVOR';

        // Reject duplicate
        $existing = $this->findExistingRecord($formType, $date, $shiftType);
        if ($existing) {
            throw new TfpGensetDvorDuplicateException($existing);
        }
        ...
        return DB::transaction(function () use (
            ...
            $record = TfpGensetDvorRecord::create([
                ...
                'location'        => $location,
                'columns_config'  => TfpGensetDvorTemplate::defaultColumnsConfig(),
                'status'          => 'ongoing',
            ...
            foreach ($rosterContext['technicians'] as $tech) {
                TfpGensetDvorTechnician::create([
                    'genset_dvor_record_id' => $record->id,
            ...
            $itemRows = TfpGensetDvorTemplate::buildItemRows($record->id);
            if (!empty($itemRows)) {
                TfpGensetDvorItem::insert($itemRows);
            }

            // Seed facilities from template
            $facilityRows = TfpGensetDvorTemplate::buildFacilityRows($record->id);
            if (!empty($facilityRows)) {
                TfpGensetDvorFacility::insert($facilityRows);
            }
    ...
    private function resolveRosterContext(string $shiftType, string $date): array
    {
        ...
        try {
            ...
            Log::warning('TfpGensetDvorService: roster lookup failed', [
                'shift_type' => $shiftType,
                'date'       => $date,
                'error'      => $e->getMessage(),
            ]);
    ...
    /**
     * Generate a sequential form number for TFP Genset DVOR records.
     *
     * Format: TFP-GENSETDVOR-{YYMMDD}-{SEQ}
     * Example: TFP-GENSETDVOR-260519-001
     *
     * Rules:
     *   - Prefix: always "TFP-GENSETDVOR"
    ...
    public function generateFormNumber(string $date): string
    {
        ...
        $prefix = 'TFP-GENSETDVOR-' . $dateYymmdd;

        $count = TfpGensetDvorRecord::withTrashed()
     ...
     */
    public function updateItems(TfpGensetDvorRecord $record, array $items, ?string $timeOverride = null): TfpGensetDvorRecord
    {
     ...
     * @return string[]
     */
    private function cellKeysFromConfig(TfpGensetDvorRecord $record): array
    {
     ...
     * }> $facilities
     */
    public function updateFacilities(TfpGensetDvorRecord $record, array $facilities, ?string $timeOverride = null): TfpGensetDvorRecord
    {
        if ($record->status === 'completed') {
            throw new RuntimeException('Form yang sudah completed tidak dapat diubah lagi.');
        }
        ...
        return DB::transaction(function () use ($record, $facilities, $timeOverride) {
            ...
            foreach ($facilities as $payload) {
                ...
                /** @var TfpGensetDvorFacility $facility */
    ...
    /**
     * Update the Genset-specific fields that have no equivalent on the other
     * TFP modules: free-text catatan, Status Operasi (PLN OFF / RUN UP),
     * Status (Master / Slave), and the fuel gauge reading. Only the keys
     * present in $data are touched (partial update).
     *
     * @param array{catatan?:string|null, status_operasi?:string|null, status_master_slave?:string|null, fuel_level?:string|null} $data
     */
    public function updateGensetFields(TfpGensetDvorRecord $record, array $data): TfpGensetDvorRecord
    {
        if ($record->status === 'completed') {
            throw new RuntimeException('Form yang sudah completed tidak dapat diubah lagi.');
        }
        ...
        $patch = array_intersect_key($data, array_flip([
            'catatan', 'status_operasi', 'status_master_slave', 'fuel_level',
        ]));
    ...
    public function addParameter(TfpGensetDvorRecord $record, array $data): TfpGensetDvorRecord
    {
        if ($record->status === 'completed') {
        ...
        TfpGensetDvorItem::create([
            ...
            'group_label'          => isset($data['group_label']) && $data['group_label'] !== '' ? trim((string) $data['group_label']) : null,
    ...
    public function updateParameterStructure(TfpGensetDvorRecord $record, int $paramId, array $data): TfpGensetDvorRecord
    {
        if ($record->status === 'completed') {
            throw new RuntimeException('Form yang sudah completed tidak dapat diubah strukturnya.');
        }

        /** @var TfpGensetDvorItem|null $item */
        ...
        if (array_key_exists('group_label', $data)) {
            $patch['group_label'] = $data['group_label'] !== null && $data['group_label'] !== ''
                ? trim((string) $data['group_label']) : null;
        }
    ...
    public function deleteParameter(TfpGensetDvorRecord $record, int $paramId): TfpGensetDvorRecord
    {
        if ($record->status === 'completed') {
            throw new RuntimeException('Form yang sudah completed tidak dapat diubah strukturnya.');
        }
     ...
     */
    public function reorderParameters(TfpGensetDvorRecord $record, array $orderedIds): TfpGensetDvorRecord
    {
        if ($record->status === 'completed') {
            throw new RuntimeException('Form yang sudah completed tidak dapat diubah strukturnya.');
        }
    ...
    public function addFacility(TfpGensetDvorRecord $record, array $data): TfpGensetDvorRecord
    {
        if ($record->status === 'completed') {
            throw new RuntimeException('Form yang sudah completed tidak dapat diubah strukturnya.');
        }
    ...
    public function updateFacilityStructure(TfpGensetDvorRecord $record, int $facilityId, array $data): TfpGensetDvorRecord
    {
        if ($record->status === 'completed') {
            throw new RuntimeException('Form yang sudah completed tidak dapat diubah strukturnya.');
        }

        /** @var TfpGensetDvorFacility|null $facility */
    ...
    public function deleteFacility(TfpGensetDvorRecord $record, int $facilityId): TfpGensetDvorRecord
    {
        if ($record->status === 'completed') {
            throw new RuntimeException('Form yang sudah completed tidak dapat diubah strukturnya.');
        }
     ...
     * @param  array<int, int>  $orderedIds
     */
    public function reorderFacilities(TfpGensetDvorRecord $record, array $orderedIds): TfpGensetDvorRecord
    {
        if ($record->status === 'completed') {
            throw new RuntimeException('Form yang sudah completed tidak dapat diubah strukturnya.');
        }
    ...
    private function fresh(TfpGensetDvorRecord $record): TfpGensetDvorRecord
    {
        return $record->fresh([
            'technicians',
            'items',
     ...
     */
    public function saveStructure(TfpGensetDvorRecord $record, array $columnsConfig, array $itemPatches): TfpGensetDvorRecord
    {
        if ($record->status === 'completed') {
            throw new RuntimeException('Form yang sudah completed tidak dapat diubah strukturnya.');
        }
    ...
    private function signTechnicianRow(
        ...
        /** @var TfpGensetDvorTechnician|null $row */
        $row = null;
        if ($technicianRowId) {
        ...
        if (!$row) {
            $row = $record->technicians()
                ->get()
                ->first(fn (TfpGensetDvorTechnician $t) => WorkOrderService::namesMatch($t->technician_name, $signer->name));
        }
    ...
    public function deleteRecord(TfpGensetDvorRecord $record): void
    {
        $record->delete();
    }
}