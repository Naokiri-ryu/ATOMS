...
use App\Models\Tfp\TfpGensetRadarFacility;
use App\Models\Tfp\TfpGensetRadarItem;
use App\Models\Tfp\TfpGensetRadarRecord;
use App\Models\Tfp\TfpGensetRadarTechnician;
...
class TfpGensetRadarService
{
    ...
    {
        $query = TfpGensetRadarRecord::query()
            ->with([
                'technicians:id,genset_radar_record_id,technician_id,technician_name,technician_signature,sort_order',
                'manager:id,name',
                'supervisor:id,name',
            ])
    ...
    public function findRecord(int $id): ?TfpGensetRadarRecord
    {
        return TfpGensetRadarRecord::query()
            ->with([
                'technicians',
                'items',
                'facilities',
    ...
    public function findExistingRecord(string $formType, string $date, string $shiftType): ?TfpGensetRadarRecord
    {
        return TfpGensetRadarRecord::query()
    ...
    public function createRecord(array $data, ?LocalUser $creator = null): TfpGensetRadarRecord
    {
        $formType  = $data['form_type']  ?? 'GENSET-RADAR';
        $date      = $data['date'];
        $shiftType = $data['shift_type'];
        $location  = $data['location']   ?? 'GENSET RADAR';

        $existing = $this->findExistingRecord($formType, $date, $shiftType);
        if ($existing) {
            throw new \App\Exceptions\TfpGensetRadarDuplicateException($existing);
        }
        ...
        return DB::transaction(function () use ($formType, $date, $shiftType, $location, $creator, $rosterContext) {
            ...
            $record = TfpGensetRadarRecord::create([
                ...
                'location'        => $location,
                'columns_config'  => TfpGensetRadarTemplate::defaultColumnsConfig(),
                'status'          => 'ongoing',
                'engine'          => $data['engine'] ?? 'DEUTZ',
                'alternator'      => $data['alternator'] ?? 'LEROY SUMMER',
                'kapasitas'       => $data['kapasitas'] ?? '150 KVA',
            ...
            foreach ($rosterContext['technicians'] as $tech) {
                TfpGensetRadarTechnician::create([
                    'genset_radar_record_id' => $record->id,
            ...
            $itemRows = TfpGensetRadarTemplate::buildItemRows($record->id);
            if (!empty($itemRows)) {
                TfpGensetRadarItem::insert($itemRows);
            }

            $facilityRows = TfpGensetRadarTemplate::buildFacilityRows($record->id);
            if (!empty($facilityRows)) {
                TfpGensetRadarFacility::insert($facilityRows);
            }
    ...
    private function resolveRosterContext(string $shiftType, string $date): array
    {
        ...
        try {
            ...
            Log::warning('TfpGensetRadarService: roster lookup failed', [
                'shift_type' => $shiftType,
                'date'       => $date,
                'error'      => $e->getMessage(),
            ]);
    ...
    public function generateFormNumber(string $date): string
    {
        ...
        $prefix     = 'TFP-GENSETRADAR-' . $dateYymmdd;

        $count = TfpGensetRadarRecord::withTrashed()
    ...
    public function updateItems(TfpGensetRadarRecord $record, array $items, ?string $timeOverride = null): TfpGensetRadarRecord
    {
    ...
    private function cellKeysFromConfig(TfpGensetRadarRecord $record): array
    {
    ...
    public function updateFacilities(TfpGensetRadarRecord $record, array $facilities, ?string $timeOverride = null): TfpGensetRadarRecord
    {
        if ($record->status === 'completed') {
            throw new RuntimeException('Form yang sudah completed tidak dapat diubah lagi.');
        }
    ...
    public function updateGensetFields(TfpGensetRadarRecord $record, array $data): TfpGensetRadarRecord
    {
        if ($record->status === 'completed') {
            throw new RuntimeException('Form yang sudah completed tidak dapat diubah lagi.');
        }
    ...
    public function addParameter(TfpGensetRadarRecord $record, array $data): TfpGensetRadarRecord
    {
        if ($record->status === 'completed') {
    ...
    public function updateParameterStructure(TfpGensetRadarRecord $record, int $paramId, array $data): TfpGensetRadarRecord
    {
        if ($record->status === 'completed') {
            throw new RuntimeException('Form yang sudah completed tidak dapat diubah strukturnya.');
        }
    ...
    public function deleteParameter(TfpGensetRadarRecord $record, int $paramId): TfpGensetRadarRecord
    {
        if ($record->status === 'completed') {
            throw new RuntimeException('Form yang sudah completed tidak dapat diubah strukturnya.');
        }
    ...
    public function reorderParameters(TfpGensetRadarRecord $record, array $orderedIds): TfpGensetRadarRecord
    {
        if ($record->status === 'completed') {
            throw new RuntimeException('Form yang sudah completed tidak dapat diubah strukturnya.');
        }
    ...
    public function addFacility(TfpGensetRadarRecord $record, array $data): TfpGensetRadarRecord
    {
        if ($record->status === 'completed') {
            throw new RuntimeException('Form yang sudah completed tidak dapat diubah strukturnya.');
        }
    ...
    public function updateFacilityStructure(TfpGensetRadarRecord $record, int $facilityId, array $data): TfpGensetRadarRecord
    {
        if ($record->status === 'completed') {
            throw new RuntimeException('Form yang sudah completed tidak dapat diubah strukturnya.');
        }
    ...
    public function deleteFacility(TfpGensetRadarRecord $record, int $facilityId): TfpGensetRadarRecord
    {
        if ($record->status === 'completed') {
            throw new RuntimeException('Form yang sudah completed tidak dapat diubah strukturnya.');
        }
    ...
    public function reorderFacilities(TfpGensetRadarRecord $record, array $orderedIds): TfpGensetRadarRecord
    {
        if ($record->status === 'completed') {
            throw new RuntimeException('Form yang sudah completed tidak dapat diubah strukturnya.');
        }
    ...
    public function saveStructure(TfpGensetRadarRecord $record, array $columnsConfig, array $itemPatches): TfpGensetRadarRecord
    {
        if ($record->status === 'completed') {
            throw new RuntimeException('Form yang sudah completed tidak dapat diubah strukturnya.');
        }
    ...
    private function signRecordRole(
        ...
        if (!$expectedName) {
            throw new \App\Exceptions\SignerNotAuthorizedException(
    ...
    private function signTechnicianRow(
        ...
        if (!$row) {
            $row = $record->technicians()
                ->get()
                ->first(fn (TfpGensetRadarTechnician $t) => WorkOrderService::namesMatch($t->technician_name, $signer->name));
        }
    ...
    public function deleteRecord(TfpGensetRadarRecord $record): void
    {
        $record->delete();
    }

    private function fresh(TfpGensetRadarRecord $record): TfpGensetRadarRecord
    {
        return $record->fresh([
            'technicians',
            'items',
