...
/**
 * TfpGensetDvorRecord — header for one TFP Performance Check Genset DVOR
 * Teknik Fasilitas Penunjang form.
 *
 * Signature contract (mirrors TfpAobGroundRecord / CnsdRecorderMeterRecord):
 ...
 *   - Technician signatures live on tfp_genset_dvor_technicians.
...
class TfpGensetDvorRecord extends Model
{
    ...
    public const STATUS_OPERASI       = ['PLN_OFF', 'RUN_UP'];
    public const STATUS_MASTER_SLAVE  = ['Master', 'Slave'];
    public const FUEL_LEVELS          = ['E', '1/4', '1/2', '3/4', 'F'];
    ...
    {
        return $this->hasMany(TfpGensetDvorTechnician::class, 'genset_dvor_record_id')
            ->orderBy('sort_order')
            ->orderBy('id');
    }
    ...
    {
        return $this->hasMany(TfpGensetDvorItem::class, 'genset_dvor_record_id')
            ->orderBy('sort_order')
            ->orderBy('id');
    }
    ...
    {
        return $this->hasMany(TfpGensetDvorFacility::class, 'genset_dvor_record_id')
            ->orderBy('sort_order')
            ->orderBy('id');
    }
