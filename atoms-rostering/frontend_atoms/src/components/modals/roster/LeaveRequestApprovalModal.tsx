...
const LeaveRequestApprovalModal: React.FC<LeaveRequestApprovalModalProps> = ({
  ...
  return (
    <Modal
      ...
      size="lg"
      headerClassName="bg-gradient-to-r from-navy-600 to-navy-800 text-white"
    >
      <div className="space-y-4 sm:space-y-6">
        {isFetchingDetail && (
          <div className="flex items-center gap-2 rounded-lg border border-navy-100 bg-navy-50 px-3 py-2 text-sm text-navy-700">
        ...
        <div className="bg-navy-50 border border-navy-100 rounded-xl p-4">
          <h3 className="text-sm font-semibold text-navy-700 mb-3">Informasi Karyawan</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
            <div>
              <p className="text-xs text-gray-500">Nama</p>
              <p className="text-sm font-medium text-slate-900 break-words">{activeLeaveRequest.employee?.user?.name}</p>
            </div>
            <div>
              <p className="text-xs text-gray-500">Email</p>
              <p className="text-sm font-medium text-slate-900 break-all">{activeLeaveRequest.employee?.user?.email}</p>
            </div>
          </div>
        </div>

        <div className="bg-white border border-navy-100 rounded-xl p-4 space-y-4">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3">
            <div>
              <p className="text-xs text-gray-500">Jenis Cuti</p>
              <p className="text-sm font-semibold text-navy-700">{getLeaveTypeLabel()}</p>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div className="bg-navy-50 rounded-lg p-3 border border-navy-100">
              ...
              <p className="text-sm font-medium text-slate-900">{formatDate(activeLeaveRequest.start_date)}</p>
            </div>
            <div className="bg-navy-50 rounded-lg p-3 border border-navy-100">
              <p className="text-xs text-gray-500">Tanggal Selesai</p>
              <p className="text-sm font-medium text-slate-900">{formatDate(activeLeaveRequest.end_date)}</p>
            </div>
            <div className="bg-navy-50 rounded-lg p-3 border border-navy-100">
              <p className="text-xs text-gray-500">Total Hari</p>
              <p className="text-sm font-medium text-slate-900">{activeLeaveRequest.total_days} hari</p>
            </div>
          </div>
          ...
          {activeLeaveRequest.reason && (
            <div>
              ...
              <p className="text-sm text-slate-900 bg-navy-50 border border-navy-100 p-3 rounded-lg break-words">{activeLeaveRequest.reason}</p>
            </div>
          )}
          ...
          {activeLeaveRequest.approval_notes && activeLeaveRequest.status !== 'pending' && (
            <div>
              ...
              <p className="text-sm text-slate-900 bg-navy-50 border border-navy-100 p-3 rounded-lg break-words">{activeLeaveRequest.approval_notes}</p>
            </div>
          )}
            ...
            <div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                <button
                  ...
                  className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-navy-700 text-white text-sm font-medium hover:bg-navy-800 disabled:opacity-60 transition-colors"
                >
        ...
        <div className="bg-white border border-navy-100 rounded-xl p-4 space-y-3">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
            <div>
              <h3 className="text-sm font-semibold text-navy-700">Manager Penanggung Jawab per Tanggal</h3>
          ...
          <div className="space-y-3">
            {approvalDates.length > 0 ? approvalDates.map((approval, index) => (
              <div key={`${approval.work_date ?? approval.label ?? 'approval'}-${index}`} className="rounded-xl border border-navy-100 bg-navy-50 p-3">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                  <div className="min-w-0 space-y-2">
                    <div>
                      <p className="text-xs text-gray-500">Tanggal</p>
                      <p className="text-sm font-semibold text-navy-700 break-words">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {approval.employee_shift_notes && (
                        <span className="inline-flex items-center rounded-full bg-navy-100 px-2.5 py-1 text-[11px] font-medium text-navy-700">
                    ...
                    <div>
                      ...
                      {approval.manager ? (
                        <div className="space-y-0.5">
                          <p className="text-sm font-medium text-slate-900 break-words">{approval.manager.name}</p>
              ...
              <div className="rounded-lg border border-dashed border-navy-100 p-4 text-sm text-gray-500">
          ...
          <div className={`p-4 rounded-xl ${actionType === 'approve' ? 'bg-emerald-50 border border-emerald-200' : 'bg-red-50 border border-red-200'}`}>
            ...
            <div>
              ...
              <textarea
                ...
                rows={4}
                className="w-full px-3 py-2 border border-navy-100 rounded-md focus:outline-none focus:ring-2 focus:ring-navy-600 text-sm"
